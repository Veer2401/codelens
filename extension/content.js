...
class CodeLensAnalyzer {
  ...
  setup() {
    ...
    chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
      ...
      switch (request.action) {
        ...
        case 'generatePDF':
          // Generate PDF report
          console.log('CodeLens: PDF generation requested from popup')
          this.generatePDFReport().then(() => {
            sendResponse({ success: true })
          }).catch(error => {
            console.error('CodeLens: PDF generation error:', error)
            sendResponse({ success: false, error: error.message })
          })
          return true
  ...
  createFloatingWidget() {
    ...
    widget.className = 'codelens-widget codelens-collapsed'
    widget.innerHTML = `
    ...
    widget.classList.add('codelens-collapsed')
    ...
    // Add PDF generation button functionality
    const pdfBtn = widget.querySelector('#codelens-pdf')
    if (pdfBtn) {
  ...
  makeIconDraggable(widget, iconBtn) {
    let isDragging = false
    let startX, startY, startLeft, startTop
    
    iconBtn.addEventListener('mousedown', (e) => {
      // Only start dragging, don't toggle
      isDragging = false
      startX = e.clientX
      startY = e.clientY
  ...
  saveWidgetPosition(pos) {
    try {
      localStorage.setItem('codelens_widget_pos', JSON.stringify(pos))
    } catch (e) {
      console.log('CodeLens: Could not save position')
    }
  }

  loadWidgetPosition() {
    try {
      const saved = localStorage.getItem('codelens_widget_pos')
  ...
  updateFloatingWidget() {
    ...
    if (complexityElement) {
      ...
      complexityElement.textContent = avgComplexity.toFixed(1)
    }
  ...
  async generatePDFReport() {
    try {
      console.log('CodeLens: Starting PDF generation...')
      
      // jsPDF is already loaded via manifest.json
      if (typeof window.jspdf === 'undefined') {
        throw new Error('jsPDF library not loaded')
      }
      
      console.log('CodeLens: jsPDF available:', typeof window.jspdf)
      
      const { jsPDF } = window.jspdf
      const doc = new jsPDF()
      
      console.log('CodeLens: PDF document created')
      
      // Get the current code and file information
      const { code } = this.getPrimaryCodeText()
      ...
      doc.text('CodeLens Analysis Report', margin, yPos)
      yPos += 15
      ...
      doc.text(`File Name: ${fileName}`, margin, yPos)
      yPos += 6
      doc.text(`Language: ${this.complexityData.language || 'Unknown'}`, margin, yPos)
      yPos += 6
      ...
      doc.setFontSize(11)
      doc.setTextColor(0, 0, 0)
      doc.text(`Total Functions: ${this.complexityData.totalFunctions}`, margin, yPos)
      yPos += 6
      doc.text(`Average Complexity: ${avgComplexity.toFixed(1)}`, margin, yPos)
      yPos += 6
      doc.text(`Code Health Score: ${healthScore}%`, margin, yPos)
      yPos += 10
      ...
      // Health fill
      const fillWidth = (healthScore / 100) * barWidth
      ...
      doc.setFontSize(11)
      const lowCount = this.complexityData.functions.filter(f => f.complexity <= 10).length
      const mediumCount = this.complexityData.functions.filter(f => f.complexity > 10 && f.complexity <= 20).length
      const highCount = this.complexityData.functions.filter(f => f.complexity > 20 && f.complexity <= 35).length
      const extremeCount = this.complexityData.functions.filter(f => f.complexity > 35).length
      
      doc.setTextColor(16, 185, 129)
      doc.text(`Low (1-10): ${lowCount} functions`, margin, yPos)
      yPos += 6
      doc.setTextColor(59, 130, 246)
      doc.text(`Medium (11-20): ${mediumCount} functions`, margin, yPos)
      yPos += 6
      doc.setTextColor(245, 158, 11)
      doc.text(`High (21-35): ${highCount} functions`, margin, yPos)
      yPos += 6
      doc.setTextColor(239, 68, 68)
      doc.text(`Extreme (36+): ${extremeCount} functions`, margin, yPos)
      yPos += 12
      ...
      doc.setFontSize(16)
      doc.setTextColor(37, 99, 235)
      doc.text('Function Details', margin, yPos)
      yPos += 10
      
      doc.setFontSize(10)
      this.complexityData.functions.forEach((func, index) => {
        checkAddPage(15)
        
        // Set color based on complexity
        if (func.complexity <= 10) doc.setTextColor(16, 185, 129)
        else if (func.complexity <= 20) doc.setTextColor(59, 130, 246)
        else if (func.complexity <= 35) doc.setTextColor(245, 158, 11)
        else doc.setTextColor(239, 68, 68)
        
        const funcName = func.name || `Anonymous Function ${index + 1}`
        const funcText = `${funcName} - Complexity: ${func.complexity} (${func.label || 'Unknown'})`
        doc.text(funcText, margin, yPos)
        yPos += 5
        
        if (func.line) {
          doc.setTextColor(100, 100, 100)
          doc.setFontSize(8)
          doc.text(`Line: ${func.line}`, margin + 5, yPos)
          doc.setFontSize(10)
          yPos += 5
        }
      })
      ...
      const codeText = code || 'No code available'
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        doc.text(
          `Generated by CodeLens - Page ${i} of ${totalPages}`,
          pageWidth / 2,
          pageHeight - 10,
          { align: 'center' }
        )
      ...
      // Save the PDF
      doc.save(`CodeLens-Report-${fileName}-${new Date().getTime()}.pdf`)
      
      console.log('CodeLens: PDF Report generated successfully!')
      
    } catch (error) {
      console.error('CodeLens: Error generating PDF report:', error)
      console.error('CodeLens: Error stack:', error.stack)
  ...
  getComplexityColorClass(complexity) {
    ...
    if (complexity <= 35) return 'complexity-high'
  ...
  ensureWidgetVisibility() {
    ...
    // Don't reposition if widget is collapsed (icon mode)
    if (this.floatingWidget.classList.contains('codelens-collapsed')) {
      return
    }
